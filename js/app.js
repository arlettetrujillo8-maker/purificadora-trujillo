(() => {
  ...
  const APP_SCRIPT_BUILD = "20260814-simplified-container-flow";
  ...
  function renderDashboard() {
    ...
    if (can("view_own_cash")) {
      ...
      dashboardCards.push(`<article class="operational-card"><div class="operational-card-head"><div><span class="eyebrow">CAJA</span><h3>${open ? "Caja abierta" : "Caja cerrada"}</h3></div><strong>${open ? money(expected) : ""}</strong></div><div class="quick-actions"><button class="secondary-btn" data-go="caja">Ver</button><button class="primary-btn" id="dashboardCashAction">${open ? "Cerrar" : "Abrir caja"}</button></div></article>`);
    }
    if (can("rounds")) {
