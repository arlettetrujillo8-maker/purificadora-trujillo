...
begin
  ...
  select (
    coalesce((
      ...
      from public.maintenance_events event
      where event.event_type = 'production'
    ...
    coalesce((
      select sum(
        greatest(sale.quantity - coalesce(returned.quantity, 0), 0)
      )
